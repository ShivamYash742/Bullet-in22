...
export default function NewInterviewPage() {
  ...
  const uploadResume = async () => {
    ...
    try {
      ...
      console.log('📤 Sending to process-resume API...');
  ...
  const createInterview = async () => {
    ...
    // Check if selected mentor is available
    const mentor = mentors.find(m => m.id === selectedMentor);
    if (!mentor?.available) {
      alert('Selected mentor is not available. Please choose June.');
      return;
    }
                ...
                {mentors.map((mentor) => (
                  <div
                    ...
                    className={cn(
                      ...
                      selectedMentor === mentor.id && mentor.available
                  ...
                  >
                    <div className="flex flex-col items-center space-y-2">
                      ...
                      <p className={cn(
                        ...
                        !mentor.available && "text-muted-foreground"
                      )}>
                        {mentor.name}
                      </p>
                      {mentor.specialty && (
                        <p className={cn(
                          "text-xs text-center",
                          mentor.available ? "text-muted-foreground" : "text-muted-foreground/60"
                        )}>
                          {mentor.specialty}
                        </p>
                      )}
